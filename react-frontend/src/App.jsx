...
function App() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const resp = await fetch("http://localhost:8081/api/analyze/", {
        method: "POST",
        body: formData
      })
      
      if (!resp.ok) {
        throw new Error("Failed to connect to backend. Please ensure the FastAPI server is running on port 8081.")
      }
